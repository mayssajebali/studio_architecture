...
$stmt = $pdo->prepare("SELECT * FROM projets_portfolio WHERE categorie = 'residential' ORDER BY date_creation DESC");
$stmt->execute();
...
<head>
    ...
    <title>Design Résidentiel - Réalisations</title>
...
<section class="hero-section">
    <div class="container">
        <h1 class="display-4 mb-4">Nos Réalisations Résidentielles</h1>
        <p class="lead">Découvrez notre portfolio de projets résidentiels</p>
    </div>
</section>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="project-media">
                            <?php if ($p['type_media'] === 'image'): ?>
                                <img src="<?= htmlspecialchars($p['media_path']) ?>" alt="<?= htmlspecialchars($p['titre']) ?>">
                            <?php else: ?>
                                <video controls><source src="<?= htmlspecialchars($p['media_path']) ?>" type="video/mp4"></video>
                            <?php endif; ?>
                        </div>
                        <div class="project-info">
                            ...
                            <p><?= nl2br(htmlspecialchars($p['description_longue'])) ?></p>
                            <div class="row">
                                <div class="col-md-6"><h5>Concept</h5><p><?= nl2br(htmlspecialchars($p['concept'])) ?></p></div>
                                <div class="col-md-6"><h5>Matériaux</h5><p><?= nl2br(htmlspecialchars($p['materiaux'])) ?></p></div>
                            </div>
                            <div class="project-details">
                                <h5>Détails</h5>
                                <div class="row">
                                    <div class="col-md-4"><strong>Superficie:</strong> <?= htmlspecialchars($p['superficie']) ?></div>
                                    <div class="col-md-4"><strong>Durée:</strong> <?= htmlspecialchars($p['duree']) ?></div>
                                    <div class="col-md-4"><strong>Budget:</strong> <?= htmlspecialchars($p['budget']) ?></div>
                                    <div class="col-md-4"><strong>Architecte:</strong> <?= htmlspecialchars($p['architecte']) ?></div>
                                    <div class="col-md-4"><strong>Designer:</strong> <?= htmlspecialchars($p['designer']) ?></div>
                                    <div class="col-md-4"><strong>Photographe:</strong> <?= htmlspecialchars($p['photographe']) ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
